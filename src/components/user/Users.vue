...
export default {
  data () {
    ...
    var checkMobile = (rule, value, cb) => {
      // 验证手机号的正则表达式
      const regMobile = /^(0|86|17951)?(13[0-9]|15[0123456789]|14[57])[0-9]{8}$/
    ...
    return {
      ...
      editFormRules: {
        ...
        mobile: [
          { required: true, message: '请输入用户的电话号', trigger: 'blur' },
  ...
  methods: {
    ...
    editUserInfo () {
      this.$refs.editFormRef.validate(async valid => {
        // console.log(valid)
        if (!valid) return this.$message.error('不合法信息')
        // 发起修改用户信息的数据请求
